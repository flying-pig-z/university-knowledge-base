...
#### 2.和c++一样类中对象可以没有初始化
为什么要刻意强调这一点呢？因为与c++[不同的](https://so.csdn.net/so/search?q=%E4%B8%8D%E5%90%8C%E7%9A%84&spm=1001.2101.3001.7020)是java方法中建立变量没有初始化会报错。但是类中对象没有初始化会像c++一样分配初始值。  
...
补充：  
Java中没有析构[函数](https://marketing.csdn.net/p/3127db09a98e0723b83b2914d9256174?pId=2782&utm_source=glcblog&spm=1001.2101.3001.7020)。  
...
#### 6.类的分类
【1】Java中专门用来**描述一类事物**的类就叫做**Javabean类**，简单说就是里面没有main方法。  
...
![](https://cdn.nlark.com/yuque/0/2024/png/42768076/1734410296359-01a6f298-638b-4040-b68d-040fab172d4b.png)
...
### 十四.Object和[String](https://marketing.csdn.net/p/3127db09a98e0723b83b2914d9256174?pId=2782&utm_source=glcblog&spm=1001.2101.3001.7020)类
### 1.Object类
#### 【1】Object介绍
...
equals方法没有重写则等效于equals方法，有重写经常用于比较两个对象的属性是否相等。  
比如比较两个String和包装类型的值是否相等经常使用equals方法。

#### 【3】hashCode方法
hashCode()的作用是获取哈希码。在Java与哈希表相关的容器HashMap和HashSet底层实现就会调用这个方法。
...
#### 【2】比较
String是不可变的，而StringBuffer线程有加同步锁，是线程安全的，但是相对[性能](https://marketing.csdn.net/p/3127db09a98e0723b83b2914d9256174?pId=2782&utm_source=glcblog&spm=1001.2101.3001.7020)低一点点。  
而StringBuilder是线程不安全的，而性能高一点点。
...
通过String的定义可以看到String里的字符数组被final和private修饰：  
*保存字符串的数组是私有的，并且String 类没有提供/暴露修改这个字符串的方法。  
...
java8及以前的字符串拼接+底层使用的是StringBuilder的append()方法实现的，拼接完成之后调用 toString() 得到一个 String 对象 。但是会有重复创建StringBuilder 对象的缺点，所以Java8之前会自己创建StringBuilder进行拼接效率比较高。
